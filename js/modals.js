...
// Глобальный контекст для журналов
    ...
    const setupEventDelegation = () => {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (journalCard) {
                ...
                if (journalName) {
                    window._journalContext = { journalName };
                    renderCurrentPage();
                }
                return;
            }
            ...
            if (e.target.closest('#addTaskToJournal')) {
                e.preventDefault();
                const journalName = window._journalContext?.journalName;
            ...
            if (e.target.closest('#showAllTasksFromJournals')) {
                e.preventDefault();
                window._journalContext = { showAllTasks: true };
                renderCurrentPage();
                return;
            }
            ...
            if (filterChip) {
                ...
                if (filter === 'all') {
                    // Обходим приватность Calendar._activeFilters через прямое присвоение
                    // В реальном проекте лучше сделать метод setAllFilters
                    const allJournals = Data.journals().map(j => j.name);
                    // Используем toggleFilter для каждого
                    const currentFilters = Calendar.getActiveFilters();
                    allJournals.forEach(j => {
                        if (!currentFilters.includes(j)) {
                            Calendar.toggleFilter(j);
                        }
                    });
                } else {
